...
var CONNECT_API_KEY = "RKOTRRRAOquK5xqdWJyVp8LW704mgwovmD1FUzOif5R61XGRyfW_X_2pJgt_fIjo"; // LinkedIn API Key
...
var executeCode = function() {
  ...
  try {
    if($includeButtons.is(":checked")) {
      var style = "border: 1px dashed #adadad; position:fixed; right:10px; top:0; padding:10px 10px 10px 10px";
...
var throwErrorMessage = function( id, message, type ) {
  ...
  if (message.indexOf('unused onLoad') == -1) {
	console.log(errType+message);
  	...
  	if( $errID.length > 0 ) {
    	...
    	$errorContainer.append('<div style="max-width: 500px;" class="ui-widget" id="'+id+'" style="display:none;"><div class="ui-state-'+type+' ui-corner-all"><p><span class="ui-icon ui-icon-'+iconType+'"></span><strong>'+errType+'</strong><span class="error-message">'+message.replace(/\n/g,"<br/>")+'</span></p></div></div>')
     	 .find("#"+id).fadeIn("fast");
  	}
  }
}
...
$("#examples").load("examples/examples.html", function() {
  ...
  $("a",".example-group").click( function(event) {
    ...
    $("a", ".example-group").each(function() {       
        $(this).parent().removeClass("ui-widget ui-selected-example ui-corner-all")
    })
    $(this).parent().addClass("ui-widget ui-selected-example ui-corner-all")
    loc.hash="#"+href;
  ...
  if (urlhref = getExampleFromHash()) {
      $("a[href*=" + urlhref + "]").parent().parent().parent().children('a').click();
      $("a[href*=" + urlhref + "]").click();
  } else {
      $("a.category", "#examples").first().click();
      $("a",".example-group").first().click();
  }
});
